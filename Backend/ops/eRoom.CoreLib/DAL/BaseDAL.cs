...
namespace eRoom.CoreLib.DAL
{
    ...
    public class BaseDAL : IBaseDAL
    {
        ...
        /// <param name="sp"></param>
        /// <param name="param"></param>
        /// <param name="paramReturn"></param>
        /// <param name="dbType"></param>
        /// <returns></returns>
        public virtual async Task<(TMetaResult, TResult)> ExecSPWithReturnAsync<TMetaResult, TResult>(string sp, object param, string paramReturn, DbType dbType) where TResult : new()
        {
            try
            {
                ...
                using (var connection = await OpenConnectionAsync())
                {
                    ...
                    dParam.Add(paramReturn, 0, dbType, ParameterDirection.Output);
                    var watch = System.Diagnostics.Stopwatch.StartNew();
                    logger.LogInformation("[{ModuleName}] Executing ExecSPWithReturnAsync sp {sp} with {param}", ModuleName, sp, JsonConvert.SerializeObject(param));
                    using (var resultSet = await connection.QueryMultipleAsync(sp, dParam, commandType: CommandType.StoredProcedure))
                    {
                        first = resultSet.ReadFirstOrDefault<TMetaResult>();
                        var count = dParam.Get<dynamic>(paramReturn);
                        second = count == null ? new TResult() : dParam.Get<TResult>(paramReturn);
                    }
                    watch.Stop();
                    logger.LogInformation("[{ModuleName}] Executed ExecSPWithReturnAsync sp {sp} with {param} in {elaspTime}ms", ModuleName, sp, JsonConvert.SerializeObject(param), watch.ElapsedMilliseconds);
                    return (first, second);
                }
            }
            catch (Exception)
