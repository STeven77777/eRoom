(function () {
    ...
    var newMotorbikeController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        var vm = this;
        var ApiUrl = $rootScope.getRootUrl() + '/Motorbikes';

        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorbikeInfoNew' });
            $http({
                method: 'GET',
        ...
        $scope.SaveInfo = function () {
            ...
            $http({
                method: 'POST',
                data: $scope._Object,
                url: ApiUrl + '/AddMotorbike'
            }).then(
                function successCallback(response) {
                    if (response.data) {
                        if (response.data.ResponseCode === 0 && response.data.Result) {
                            response.data.ResponseDesc += '. Click on <a href="' + $rootScope.getRootUrl() + "/Motorbikes#/" + response.data.Result.MotorbikeIDHash + '/Info">' + response.data.Result.MotorbikeIDHash + '</a> to view detail.'
        ...
        $scope.BackToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/";
        };
    }

    var motorbikesController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        var vm = this;
        var ApiUrl = $rootScope.getRootUrl() + '/Motorbikes';
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorbikeSearch' });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.OnSearch = function () {
            ...
            var uri = ApiUrl + '/MotorbikeSearch?' + ($scope._Object ? $.param($scope._Object) : '');
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + aData[7] + "/Info";
        });
    }

    var motorbikeInfoController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorbikeInfo', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.ShowHistory = function () {
            ...
            var opts = angular.extend($scope.stsHisLst, {
                serverSide: true,
                ajax: ApiUrl + '/AccountLogList?' + $.param({ MotorbikeIDHash: $rootScope.obj.busnLocationNoHash }),
                destroy: true
            });
    ...
    var busnLotAddrController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotAddrs', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Address/" + aData[12];
        });

        $scope.gotoAddressNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Address/New";
        };
    };
    ...
    var busnLotAddrNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotAddrNew', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Address";
        };
    ...
    var busnLotAddrDetailController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotAddrD', BusnLocationAddressId: $rootScope.obj.busnLocationAddressId, MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
    ...
    var busnLotContController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotConts', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Contact/" + aData[7];
        });

        $scope.gotoNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Contact/New";
        };
    };
    ...
    var busnLotContNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotContNew', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Contact";
        };
    ...
    var busnLotContDetailController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotContD', BusnLocationContactId: $rootScope.obj.busnLocationContactId, MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
    ...
    var busnLotTerminalsController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotTerminals', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
            ...
            .then(function () {
                if ($scope._GeneralInfo.EntityId) {
                    $scope.dtOptions = {
                        ...
                        "ordering": true,
                        id: 'allAddresses',
                        ajax: ApiUrl + '/TerminalList?' + $.param({ MotorbikeIDHash: $scope._GeneralInfo.MotorbikeIDHash }),
                        edit: {
                            level: 'scope',
                            func: 'gotoDetail'
                        },
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals/" + aData[7];
        });

        $scope.gotoNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals/New";
        };
    };
    ...
    var busnLotTerminalNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotTerminalNew', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/Motorbikes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals";
        };
    ...
    var busnLotTerminalInfoController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotTerminalInfo', TerminalIds: $rootScope.obj.terminalIds, MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
    ...
    var busnLotTxnPointsController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            ...
            var params = $.param({ Prefix: 'BusnLotTxnPoints', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
    ...
    var busnLotCrdRangeAccptController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            ...
            var params = $.param({ Prefix: 'BusnLotCrdRangeAccpt', MotorbikeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.AddCardRange = function () {
            ...
            $http({
                method: 'POST',
                data: {
                    MotorbikeIDHash: $scope._GeneralInfo.MotorbikeIDHash,
    ...
    newMotorbikeController.$inject = injectParams;
    motorbikesController.$inject = injectParams;
    motorbikeInfoController.$inject = injectParams;
    ...
    angular.module('loyaltyApp').controller('newMotorbikeController', newMotorbikeController);
    angular.module('loyaltyApp').controller('motorbikesController', motorbikesController);
    angular.module('loyaltyApp').controller('motorbikeInfoController', motorbikeInfoController);
