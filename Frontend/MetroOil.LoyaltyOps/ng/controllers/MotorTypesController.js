(function () {
    ...
    var newMotorTypeController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        var vm = this;
        var ApiUrl = $rootScope.getRootUrl() + '/MotorTypes';

        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorTypeInfoNew' });
            $http({
                method: 'GET',
        ...
        $scope.SaveInfo = function () {
            ...
            $http({
                method: 'POST',
                data: $scope._Object,
                url: ApiUrl + '/AddMotorType'
            }).then(
                function successCallback(response) {
                    if (response.data) {
                        if (response.data.ResponseCode === 0 && response.data.Result) {
                            response.data.ResponseDesc += '. Click on <a href="' + $rootScope.getRootUrl() + "/MotorTypes#/" + response.data.Result.MotorTypeIDHash + '/Info">' + response.data.Result.MotorTypeIDHash + '</a> to view detail.'
        ...
        $scope.BackToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/";
        };
    }

    var motorTypesController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        var vm = this;
        var ApiUrl = $rootScope.getRootUrl() + '/MotorTypes';
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorTypeSearch' });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.OnSearch = function () {
            ...
            var uri = ApiUrl + '/MotorTypeSearch?' + ($scope._Object ? $.param($scope._Object) : '');
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + aData[8] + "/Info";
        });
    }

    var motorTypeInfoController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http, commonService) {
        // local declare
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'MotorTypeInfo', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.ShowHistory = function () {
            ...
            var opts = angular.extend($scope.stsHisLst, {
                serverSide: true,
                ajax: ApiUrl + '/AccountLogList?' + $.param({ MotorTypeIDHash: $rootScope.obj.busnLocationNoHash }),
                destroy: true
            });
    ...
    var busnLotAddrController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotAddrs', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Address/" + aData[12];
        });

        $scope.gotoAddressNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Address/New";
        };
    };
    ...
    var busnLotAddrNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotAddrNew', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Address";
        };
    ...
    var busnLotAddrDetailController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotAddrD', BusnLocationAddressId: $rootScope.obj.busnLocationAddressId, MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
    ...
    var busnLotContController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotConts', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Contact/" + aData[7];
        });

        $scope.gotoNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Contact/New";
        };
    };
    ...
    var busnLotContNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotContNew', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Contact";
        };
    ...
    var busnLotContDetailController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotContD', BusnLocationContactId: $rootScope.obj.busnLocationContactId, MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
    ...
    var busnLotTerminalsController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        $.blockUI();
        var params = $.param({ Prefix: 'BusnLotTerminals', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
        $http({
            method: 'GET',
            url: ApiUrl + '/FillData?' + params
        })
            ...
            .then(function () {
                if ($scope._GeneralInfo.EntityId) {
                    $scope.dtOptions = {
                        ...
                        "ordering": true,
                        id: 'allAddresses',
                        ajax: ApiUrl + '/TerminalList?' + $.param({ MotorTypeIDHash: $scope._GeneralInfo.MotorTypeIDHash }),
                        edit: {
                            level: 'scope',
                            func: 'gotoDetail'
                        },
        ...
        $scope.$on("gotoDetail", function (event, aData) {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals/" + aData[7];
        });

        $scope.gotoNew = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals/New";
        };
    };
    ...
    var busnLotTerminalNewController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotTerminalNew', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.backToList = function () {
            window.location.href = $rootScope.getRootUrl() + "/MotorTypes#/" + $rootScope.obj.busnLocationNoHash + "/Terminals";
        };
    ...
    var busnLotTerminalInfoController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            $.blockUI();
            var params = $.param({ Prefix: 'BusnLotTerminalInfo', TerminalIds: $rootScope.obj.terminalIds, MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
    ...
    var busnLotTxnPointsController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            ...
            var params = $.param({ Prefix: 'BusnLotTxnPoints', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
    ...
    var busnLotCrdRangeAccptController = function ($scope, $rootScope, $location, $window, $timeout, ngDialog, Utils, $http) {
        ...
        var Initial = function () {
            ...
            var params = $.param({ Prefix: 'BusnLotCrdRangeAccpt', MotorTypeIDHash: $rootScope.obj.busnLocationNoHash });
            $http({
                method: 'GET',
                url: ApiUrl + '/FillData?' + params
            })
        ...
        $scope.AddCardRange = function () {
            ...
            $http({
                method: 'POST',
                data: {
                    MotorTypeIDHash: $scope._GeneralInfo.MotorTypeIDHash,
    ...
    newMotorTypeController.$inject = injectParams;
    motorTypesController.$inject = injectParams;
    motorTypeInfoController.$inject = injectParams;
    ...
    angular.module('loyaltyApp').controller('newMotorTypeController', newMotorTypeController);
    angular.module('loyaltyApp').controller('motorTypesController', motorTypesController);
    angular.module('loyaltyApp').controller('motorTypeInfoController', motorTypeInfoController);
