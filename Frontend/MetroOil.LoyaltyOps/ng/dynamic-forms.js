...
angular.module('dynform', [])
  .directive('dynamicForm', ['$q', '$parse', '$http', '$templateCache', '$compile', '$document', '$timeout', function ($q, $parse, $http, $templateCache, $compile, $document, $timeout) {
    var supported = {
        //  Text-based elements
        'String': { element: 'input', type: 'String', editable: true, textBased: true},
        ...
        //'datepicker': { element: 'input', type: 'datepicker', editable: true, textBased: true },
    ...
    return {
        ...
        link: function ($scope, element, attrs) {
                                  ...
                                  if (String(id).charAt(0) == '$') {
                                      ...
                                      if (field.type === 'number' || field.type === 'range') {
                                          ...
                                          if (angular.isDefined(field.autoOptions)) {
                                              ...
                                              angular.forEach(field.options, function (option, childId) {
                                                  ...
                                                  newChild.attr('value', option.ValueName);
                                                  newChild.html(option.TextName);
                                                  newElement.append(newChild);
                                          ...
                                          newElement.attr('date_time', '');
                                          newElement.attr('format', 'YYYY/MM/DD');
                                          newElement.attr('view', 'date');
                                          newElement.attr('min_view', 'date');
                                          newElement.attr('auto_close', 'true');
                                      }
                                      ...
                                      if (angular.isDefined(field.label)) {
                                          ...
                                          //  Everything else should be wrapped in a label tag.
                                              ...
                                              //newElement.attr('class', 'input-icon right')
                                              newElement = newElement.wrap('<div class="input-icon right"></div>').parent();
                                              newElement.prepend('<i class="material-icons">today</i>');

                                              newElement = newElement.wrap('<div class="form-group"></div>').parent();
                                              newElement.prepend('<label>' + field.label + '</label>');
                                              //newElement.newChild('<i class="material-icons">today</i>')
                                          }
                                          else {
                                              newElement = newElement.wrap('<div class="form-group"></div>').parent();
                                              newElement.prepend('<label>' + field.label + '</label>');
            ...
            //$compile(element.contents())($scope);
              ...
              $scope.$watch("formTemplate", function (newValue, oldValue) {
