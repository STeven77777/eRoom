...
namespace MetroOil.LoyaltyOps.Controllers
{
    ...
    public class ReceiptsController : Controller
    {
        ...
        public ActionResult Tmpl(string Prefix)
        {
            ...
            {
                case "ReceiptSearch":
                    return PartialView("List", new ReceiptModel());
                case "ReceiptInfoNew":
                    return PartialView("New", new ReceiptModel());
                case "ReceiptInfo":
                    return PartialView("Info", new ReceiptModel());
                default:
                    return PartialView();
            }
        }
        ...
        public async Task<JsonResult> FillData(string Prefix, string ReceiptIDHash, string ReceiptAddressId, string ReceiptContactId, string TerminalIds)
        {
            switch (Prefix)
            {
                case "ReceiptSearch":
                    {
                        var _infoObj = new ReceiptModel();

                        var _States = await Common.GetState(null);
                        var _infoSelects = new ReceiptModel()
                        {
                            //States = _States,
                            //DBAStates = _States
                        };

                        return Json(new FormResponseModel<ReceiptModel, ReceiptModel>()
                        {
                ...
                case "ReceiptInfoNew":
                    {
                        var _newObj = new ReceiptModel()
                        {

                        };
                        var _newSelects = new ReceiptModel()
                        {
                ...
                case "ReceiptInfo":
                    {
                        var _infoObj = await ApiClient.GetJsonAsync<APIResponseModel<ReceiptModel>>
                            ("receipts/" + Helper.Decrypt(ReceiptIDHash));

                        if (_infoObj != null && _infoObj.Result != null)
                        {
                            _infoObj.Result.ReceiptIDHash = ReceiptIDHash;
                        }
                ...
                case "ReceiptAddrs":
                    {
                        //var _info = await ApiClient.GetJsonAsync<APIResponseModel<ReceiptModel>>
                        //    ("merchant/BusinessLocation?" + Helper.GetQueryString(new { Receipt = Helper.Decrypt(ReceiptIDHash) }));
                        //var _info = await ApiClient.GetJsonAsync<APIResponseModel<ReceiptModel>>("Receipts/" + Helper.GetQueryString(new { ReceiptNumber = Helper.Decrypt(ReceiptIDHash) }));
                        var _info = await ApiClient.GetJsonAsync<APIResponseModel<ReceiptModel>>("Receipts/" + Helper.Decrypt(ReceiptIDHash));
                        //var _info = await ApiClient.GetJsonAsync<APIResponseListModel<ReceiptModel>>("Receipts/GetList");
                        //var _info = await ApiClient.GetArrayJsonAsync<APIResponseModel<List<ReceiptModel>>>("Receipts/GetList");

                        if (_info != null && _info.Result != null)
                        {
                            // _info.Result.ReceiptID = Helper.Encrypt(_info.Result.ReceiptID);
                            return Json(new FormResponseModel<ReceiptModel, ReceiptModel>()
                            {
                ...
                case "ReceiptAddrNew":
                    {
                        ...
                        {
                            //_info.Result.ReceiptIDHash = Helper.Encrypt(_info.Result.ReceiptID);
                            ...
                            return Json(new FormResponseModel<AddressModel, ReceiptModel>()
                            {
                                ResponseCode = Enums.RESPONSE_CODE_VALID,
                                ResponseDesc = "",
                                Model = _newAddress,
                ...
                //        if (_info != null && _info.Result != null && _info.ResponseCode == Enums.RESPONSE_CODE_VALID)
                //        {
                //            //_info.Result.ReceiptIDHash = Helper.Encrypt(_info.Result.AcctNo);

                //            var addrObj = await ApiClient.GetJsonAsync<APIResponseModel<AddressModel>>
                //                ("common/Address?" + Helper.GetQueryString(new { Ids = ReceiptAddressId }));
                ...
                //        if (_info != null && _info.Result != null)
                //        {
                //            _info.Result.ReceiptIDHash = Helper.Encrypt(_info.Result.Receipt);
                //            return Json(new FormResponseModel<ReceiptModel, ReceiptModel>()
                //            {
                //                ResponseCode = _info.ResponseCode,
                //                ResponseDesc = _info.ResponseDesc,
                ...
                //            return Json(new FormResponseModel<ContactModel, ReceiptModel>()
                //            {
                //                ResponseCode = Enums.RESPONSE_CODE_VALID,
                //                ResponseDesc = "",
                //                Model = _Obj,
                ...
                //                ("common/ContactById?" + Helper.GetQueryString(new { Ids = ReceiptContactId }));
                ...
                //            {
                //                ReceiptIDHash = _info.Result.ReceiptIDHash
                //            };
                ...
                //            return Json(new FormResponseModel<TerminalModel, ReceiptModel>()
                //            {
                //                ResponseCode = Enums.RESPONSE_CODE_VALID,
                //                ResponseDesc = "",
                //                Model = _Obj,
                ...
                //            if (_Obj.ResponseCode == Enums.RESPONSE_CODE_VALID)
                //            {
                //                if (_Obj.Result != null && _info.Result != null && _Obj.Result.Receipt == _info.Result.Receipt) // valid merchant and address
                //                {
                ...
                //            {
                //                //ReceiptNos = await Common.GetBusinessLocations(),
                //            };

                //            return Json(new FormResponseModel<TxnPointSearchModel, ReceiptModel>()
                //            {
                //                ResponseCode = Enums.RESPONSE_CODE_VALID,
                //                Model = _Obj,
                //                Selects = _Selects,
                ...
                //case "ReceiptCrdRangeAccpt":
                //    {
                //        var ReceiptNo = Helper.Decrypt(ReceiptIDHash);
                //        var _info = await ApiClient.GetJsonAsync<APIResponseModel<ReceiptModel>>
                //            ("merchant/BusinessLocation?" + Helper.GetQueryString(new { Receipt = ReceiptNo }));
                ...
                //                ("merchant/CardRangeList?ReceiptNo=" + ReceiptNo + "&" + Helper.GetQueryString(_params));
                ...
                //            return Json(new FormResponseModel<CardRangeAcceptanceListModel, ReceiptModel>()
                //            {
                //                ResponseCode = Enums.RESPONSE_CODE_VALID,
                //                Model = _Obj,
                //                Selects = _Selects,
        ...
        public async Task<ActionResult> ReceiptSearch(jQueryDataTableParamModel Params, ReceiptModel model)
        {
            var displyColumns = new string[] { "ReceiptID", "BillID", "PeopleID", "ReceiptAmount", "CreateDate", "CreateUser", "Description" };
            ...
            var response = await ApiClient.GetJsonAsync<APIResponseListModel<ReceiptModel>>
                ("receipts/receiptlist?" + Helper.GetQueryString(Params) + "&" + Helper.GetQueryString(model));

            if (response == null || response.ResponseCode != 0 || response.Result == null || response.Result.RecordList == null) // empty data/ error
            {
                response.Result.RecordList = new List<ReceiptModel>();
            }
            ...
            return Json(new
            {
                ...
                aaData = response.Result.RecordList.Select((x, index) => new object[] { Params.iDisplayStart + index + 1, x.ReceiptID, x.BillID, x.PeopleID, 
                    x.ReceiptAmount, x.CreateDate != null ? x.CreateDate.Value.ToString(Enums.DATE_FORMAT) : string.Empty, x.CreateUser,
                    x.Description, Helpers.Helper.Encrypt(x.ReceiptID) })
            }, JsonRequestBehavior.AllowGet);
        }
        #endregion
        ...
        [HttpPost]
        public async Task<JsonResult> AddReceipt(ReceiptModel model)
        {
            //    model.AllCardRange = model.AllCardRange ?? false;
            //    model.AllTxnCodeMapping = model.AllTxnCodeMapping ?? false;

            // Call API to update 
            var rp = await ApiClient.PostJsonAsync<APIResponseModel<ReceiptModel>>
                ("Receipts", JsonConvert.SerializeObject(model));

            if (rp.ResponseCode == Enums.RESPONSE_CODE_VALID && rp.Result != null)
            {
                rp.Result.ReceiptIDHash = Helper.Encrypt(rp.Result.ReceiptID);
            }
        ...
        [HttpPatch]
        public async Task<JsonResult> UpdateReceipt(ReceiptModel model)
        {
            //model.ReceiptID = Helper.Decrypt(model.ReceiptID);
            //var result = await ApiClient.PatchJsonAsync<APIResponseModel<ReceiptModel>>
        ...
        /// <returns></returns>
        [HttpGet]
        public async Task<ActionResult> AccountLogList(jQueryDataTableParamModel Params, string ReceiptIDHash)
        {
            var response = await ApiClient.GetJsonAsync<APIResponseListModel<LogChangeModel>>
                ("merchant/BusinessLocationLogList?Receipt=" + Helper.Decrypt(ReceiptIDHash) + "&" + Helper.GetQueryString(Params));
        ...
        /// <returns></returns>
        [HttpPatch]
        public async Task<JsonResult> ReceiptStatus(Models.Merchants.Account.MerchantAcctStatusModel model)
        {
        ...
        public async Task<ActionResult> TerminalList(jQueryDataTableParamModel Params, string ReceiptIDHash)
        {
            ...
            var response = await ApiClient.GetJsonAsync<APIResponseListModel<TerminalModel>>
                ("merchant/TerminalInventoryList?" + Helper.GetQueryString(Params) + "&ReceiptNo=" + Helper.Decrypt(ReceiptIDHash));
        ...
        //{
        //    // Call API to update 
        //    model.Receipt = Helper.Decrypt(model.ReceiptIDHash);
        //    //model.ReceiptNo = model.Receipt;// will be removed soon
        ...
        public async Task<ActionResult> GetTxnByBusinessLocation(jQueryDataTableParamModel Params, TxnPointSearchModel txnSearch)
        {
            if (txnSearch != null)
            {
                //txnSearch.Receipt = Helper.Decrypt(txnSearch.ReceiptIDHash);
            }
        ...
        //{
        //    model.ReceiptNo = Helper.Decrypt(model.ReceiptIDHash);
        //    // Call API to update 
